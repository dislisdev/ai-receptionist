...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM appointments WHERE session_id = ? AND status = 'confirmed'");
    $stmt->execute([$sid]);
...
<body>
  <h1>AI Receptionist - health check</h1>
  <table>
    <?php foreach ($checks as $label => $value): ?>
      <?php $bad = str_contains($value, 'MISSING') || str_contains($value, 'BLOCKED') || str_contains($value, 'FAIL'); ?>
      <tr>
